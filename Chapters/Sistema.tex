...
\section{Metriche per la Object Detection}
\label{sec:metrics}

Quando vengono condotti esperimenti di rilevazione di oggetti un aspetto importante è la valutazione delle prestazioni del detector; nel caso della object detection la metrica usata è chiamata \acf{map}. 

Prima di introdurre la \ac{map} è necessario introdurre due altre misure di base: la \textit{precision} e la \textit{recall}.
La \textit{precision} misura la percentuale di predizioni corrette, mentre la \textit{recall} misura la percentuale di predizioni corrette recuperate rispetto al totale.

Per la \textit{precision} la formula precisa è data da 
$$P = \frac{\text{True positive}}{\text{True positive} + \text{False positive}}$$
mentre per la \textit{recall} è data da:
$$R = \frac{\text{True positive}}{\text{True positive} + \text{False negative}}$$
Per indicare se le prestazioni di un classificatore sono buone o meno viene valutata la \textit{precision} in funzione della \textit{recall}; l'area sotto questa curva viene chiamata \ac{AP}, che quindi è definita come
$$AP = \int_{0}^{1} P(R) dR$$
Attraverso le nozioni appena date è possibile infine definire la \acf{map} come la media tra le \ac{AP} delle varie classi
...
Le telecamere utilizzate sono una \textit{PointGrey Flea3} per la parte a colori ed una \textit{FLIR-A35} per la parte termica. La prima acquisisce immagini ad una risoluzione di $640 x 480$ pixels con un \ac{FOV} di $103.6^\circ$, mentre la seconda ha una risoluzione di $320 x 256$ con un un \ac{FOV} di $39^\circ$. Come si può notare il campo visivo della telecamera RGB è più ampio di quello della telecamera termica, motivo per cui viene sacrificata parte dell'immagine visibile al fine di allineare i due fotogrammi. Il \textit{framerate} è di $20$ FPS.
...
In questa sezione presenteremo i risultati iniziali dell'addestramento di RetinaNet sui due dataset descritti in sezione \ref{sec:dataset}. Per lo scopo è stata usata una versione di \textit{RetinaNet} implementata tramite \textit{Keras} reperibile in forma originale al seguente \href{https://github.com/fizyr/keras-retinanet}{link}:\href{https://github.com/fizyr/keras-retinanet}{https://github.com/fizyr/keras-retinanet}. Durante lo sviluppo del lavoro di tesi le modifiche al codice originale sono state molteplici, tanto da aver richiesto un \textit{fork} della \textit{repository} originale reperibile al seguente \href{https://github.com/iskorini/keras-retinanet}{link}:\href{https://github.com/iskorini/keras-retinanet}{https://github.com/iskorini/keras-retinanet}. Per tenere traccia dell'addestramento è stato usato il servizio web \href{https://www.wandb.com}{\textit{Weight \& Biases}}.
...
\begin{table}[]
    ...
    Non supervisionato & Nessuna                & NO                           & NO                         & Clustering, dimensionality reduction \\ \hline
    \end{tabular}}
...
\begin{table}[]
    ...
    \end{tabular}
    \caption{Test complessivo su immagini RGB delle performance di RetinaNet dopo l'addestramento effettuato su immagini RGB}
...
\begin{table}[]
    ...
    \caption{Risultati della valutazione separata tra giorno e notte sulle immagini RGB dopo l'addestramento di RetinaNet effettuato su immagini RGB}
...
\begin{table}[]
    ...
    \end{tabular}
    \caption{Test complessivo sul termico delle performance dopo l'addestramento di RetinaNet sulle immagini termiche del dataset \ac{kmpd}}
...
\begin{table}[]
    ...
    \subfloat[Notte\label{table:separate_test_lwir_night}]{
        ...
        Cyclist     & 578               & 0.0440 \\ \hline
        Complessivo & 15219             & 0.4288 \\ \hline
        \end{tabular}
    }
    \caption{Risultati della valutazione separata tra giorno e notte, effettuata sul dataset termico di \ac{kmpd}, dopo l'addestramento di RetinaNet sulle immagini termiche del dataset \ac{kmpd}}
...
In totale le operazioni nello spazio di ricerca sono 16, ed ognuna di esse ha un intervallo di potenza discretizzato con cui può essere applicata che varia da $1$ a $10$. In maniera del tutto similare vengono discretizzati i valori di probabilità in un intervallo equispaziato di dimensione $11$. Trovare quindi una subpolicy è un problema di ricerca in uno spazio di dimensione $(16 \times 10 \times 11)^2$. Considerando che una policy contiene $5$ subpolicy lo spazio di ricerca arriva a $(16 \times 10 \times 11)^{10}$, che è nell'ordine di grandezza di $10^{32}$.
...
Al fine di ridurre il numero dei parametri, gli autori sono intervenuti anche in questo definendo un singolo valore $M$ per parametrizzare la potenza con cui vengono applicate tutte le operazioni. 

Riassumendo quindi gli iperparametri sono due: il numero $N$ di operazioni da applicare e la forza $M$ con cui verranno applicate. Essendo così pochi può essere un usato un classico algoritmo di \textit{GridSearch} per ottimizzarli. Nel caso di questa tesi è stato usato un ottimizzatore bayesiano presente sulla piattaforma web \href{https://www.comet.ml/}{Comet.ml} di cui non vengono però rilasciati dettagli tecnici o implementativi.
