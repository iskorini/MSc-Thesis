\addcontentsline{toc}{chapter}{Acronimi}
\chapter*{Acronimi}
\begin{acronym}[CAGD]
    \acro{map}[mAP]{Mean Average Precision}
    \acro{AP}[AP]{Average Precision}
    \acro{kmpd}[KAIST MPD]{KAIST Multispectral Pedestrian Dataset}
    \acro{gpu}[GPU]{Graphics Processing Unit}
    \acro{KAIST}[KAIST]{Korea Advanced Institute of Science and Technology}
    \acro{FOV}[FOV]{Field of View}
    \acro{BB}[BB]{Bounding Box}
    \acro{RNN}[RNN]{Recurrent Neural Network}
    \acro{TPU}[TPU]{Tensor Processing Unit}
    \acro{CNN}[CNN]{Convolutional Neural Network}
    \acro{FCN}[FCN]{Fully Convolutional Network}
    \acro{RNN}[RNN]{Recurrent Neural Network}
    \acro{NMS}[NMS]{Non-Maximum Suppression}
    \acro{HNM}[HNM]{Hard Negative Mining}
    \acro{RTK}[RTK]{Real-time kinematic}
    \acro{YOLO}[YOLO]{You Only Look Once}
    \acro{VOC}[VOC]{Pascal Visual Object Classes}
    \acro{ILSVRC}[ILSVRC]{ImageNet Large Scale Visual Recognition Challenge}
    \acro{MSCOCO}[MS-COCO]{Microsoft - Common Object in COntext}
    \acro{OID}[OID]{Open Images Detection}
    \acro{VJ}[VJ]{Viola Jones}
    \acro{HOG}[HOG]{Histogram of Oriented Gradients}
    \acro{DPM}[DPM]{Deformable Part-based Model}
    \acro{ROI}[ROI]{Region of Interest}
    \acro{RPN}[RPN]{Region Proposal Network}
    \acro{SVM}[SVM]{Support Vector Machines}
    \acro{IoU}[IoU]{Intersection over Union}
    \acro{SPPNet}[SPPNet]{Spatial Pyramid Pooling Networks}
    \acro{SSD}[SSD]{Single-Shot Detector}
    \acro{DSSD}[DSSD]{Deconvolutional Single-Shot Detector}
    \acro{FPN}[FPN]{Feature Pyramid Network}
    \acro{MLFPN}[MLFPN]{Multi-Level Feature Pyramid Network}
    \acro{FFM}[FFM]{Feature Fusion Module}
    \acro{TUM}[TUM]{Thinned U-shape module}
    \acro{SFAM}[SFAM]{Scale-wise Feature Aggregation Module}
    \acro{RFI}[RFI]{Rete Ferroviaria Italiana}
    \acro{VIA}[VIA]{VGG Image Annotator}
    \acro{GAN}[GAN]{Generative Adversarial Network}
\end{acronym}